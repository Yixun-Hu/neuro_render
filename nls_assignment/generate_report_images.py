...
scan_folder = "2026_03_17_16_11_46-kt3119"
...
for i, (flag, label) in enumerate([(False, "ray_offset ON"), (True, "ray_offset OFF")]):
    ...
    _, rgb, _, _ = model.generate_outputs(height=540, width=720, time=0.5, fov_scale=1.5)
    axes[i].imshow((rgb * B).clamp(0, 1).permute(1, 2, 0).cpu())
    axes[i].set_title(label, fontsize=14)
    axes[i].axis('off')
...
# --- 7. Breakdown examples (extreme FOV / edge of sphere) ---
print("=== Breakdown examples ===")
fig, axes = plt.subplots(1, 3, figsize=(18, 5))
for i, (fov, t, label) in enumerate([(3.5, 0.5, "Extreme FOV (3.5)"), (1.5, 0.0, "Edge: t=0.0"), (1.5, 1.0, "Edge: t=1.0")]):
    _, rgb, _, _ = model.generate_outputs(height=540, width=720, time=t, fov_scale=fov)
    axes[i].imshow((rgb * B).clamp(0, 1).permute(1, 2, 0).cpu())
    axes[i].set_title(label, fontsize=14)
    axes[i].axis('off')
plt.tight_layout()
