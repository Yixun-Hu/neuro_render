...
class LightSphereModel(pl.LightningModule):
    ...
    def solve_sphere_crossings(ray_origins, ray_directions):
        ...
        b = 2.0 * torch.sum(ray_origins * ray_directions, dim=1, keepdim=True)
        c = torch.sum(ray_origins * ray_origins, dim=1, keepdim=True) - 1.0

        discriminant = b * b - 4.0 * a * c
        # Take the far intersection (+ root); origins are inside the sphere
    ...
    def inference(self, t, uv, ray_origins, ray_directions, training_phase=1.0):
        ...
        if not self.args.no_offset:
            encoded_offset_position = self.encoding_offset_position((intersections_sphere * 0.5 + 0.5).clamp(0, 1))
        ...
        encoded_color_position = self.encoding_color_position((intersections_sphere_offset * 0.5 + 0.5).clamp(0, 1))
    ...
    def forward(self, t, uv, ray_origins, ray_directions, training_phase):
        ...
        if training_phase > 0.2 and not self.args.no_offset:
            encoded_offset_position = self.mask(self.encoding_offset_position((intersections_sphere * 0.5 + 0.5).clamp(0, 1)), training_phase, initial=0.2)
        ...
        encoded_color_position = self.mask(self.encoding_color_position((intersections_sphere_offset * 0.5 + 0.5).clamp(0, 1)), training_phase, initial=0.8)
...
class PanoModel(pl.LightningModule):
    ...
    def generate_ray_directions(self, uv, camera_to_world, intrinsics_inv):
        ...
        radial = 1 + kappa1 * r2 + kappa2 * r4 + kappa3 * r6
        x_distorted = x * radial + 2 * kappa4 * x * y + kappa5 * (r2 + 2 * x**2)
        y_distorted = y * radial + kappa4 * (r2 + 2 * y**2) + 2 * kappa5 * x * y
