...
class NeRF(nn.Module):
    def __init__(self, D=8, W=256, output_ch=4, skips=[4], embedder=PositionalEncoder(10,0), embedder_dirs=None, use_viewdirs=False, input_ch=3, input_ch_views=3):
        ...
        # D layers total: first layer takes input_ch, subsequent layers take W
        # At skip connection indices, the NEXT layer takes W + input_ch (concatenated input)
        ...
        if use_viewdirs:
            self.feature_linear = nn.Linear(W, W)  # Feature vector output
            self.alpha_linear = nn.Linear(W, 1)    # Volume density (sigma)
            self.rgb_linear = nn.Linear(W // 2, 3) # RGB radiance from view-dependent branch
        else:
