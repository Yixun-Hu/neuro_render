...
class Renderer(nn.Module):
    ...
    def ray_marcher(self, raw, z_vals, rays_d, pytest=False):
        ...
        def raw2alpha(raw, dists, act_fn=F.relu):
            ...
            # Volume rendering alpha: α = 1 - exp(-σ * δ)
            # act_fn(raw) ensures non-negative density (σ >= 0)
            # dists is the distance between adjacent samples (δ)
        ...
        # Transmittance T_i = Π_{j<i}(1 - α_j): probability ray reaches sample i
        # Use exclusive cumprod: pad with 1 at start, remove last element
        # Add small epsilon for numerical stability
        T = torch.cumprod(
            torch.cat([torch.ones((alpha.shape[0], 1), device=device), 1. - alpha + 1e-10], -1), -1
        )[:, :-1]
        
        # Weights w_i = T_i * α_i: contribution of each sample to final color
        weights = alpha * T  # [N_rays, N_samples]
        
        # RGB map: weighted sum of colors along ray
        rgb_map = torch.sum(weights[..., None] * rgb, -2)  # [N_rays, 3]

        # Depth map: expected depth (weighted sum of z values)
        depth_map = torch.sum(weights * z_vals, -1)  # [N_rays]
        
        # Disparity map: inverse of depth
        disp_map = 1. / torch.max(1e-10 * torch.ones_like(depth_map), depth_map / torch.sum(weights, -1))
        
        # Accumulated opacity: sum of weights (1.0 = fully opaque ray)
    ...
    def render_rays(self,
        ...
        # Compute 3D sample points along each ray: p(t) = o + t*d
